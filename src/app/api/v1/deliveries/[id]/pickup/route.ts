...
export async function POST(
  ...
  try {
    const delivery = await deliveryService.pickupDelivery(tenantId, id, auth.session.userId);
    return NextResponse.json({ delivery });
  } catch (err: any) {
    return NextResponse.json({ error: err.message || "Failed to pickup delivery" }, { status: 400 });
  }
}
