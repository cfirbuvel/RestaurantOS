...
export async function POST(req: NextRequest) {
  ...
  try {
    const driver = await driverQueueService.arrivedAtRestaurant(tenantId, branchId, auth.session.userId);
    return NextResponse.json({ driver });
  } catch (err: any) {
    return NextResponse.json({ error: err.message || "Failed to record arrival at restaurant" }, { status: 400 });
  }
}
