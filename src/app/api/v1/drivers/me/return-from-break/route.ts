...
export async function POST(req: NextRequest) {
  ...
  try {
    const driver = await driverQueueService.returnFromBreak(tenantId, branchId, auth.session.userId);
    return NextResponse.json({ driver });
  } catch (err: any) {
    return NextResponse.json({ error: err.message || "Failed to return from break" }, { status: 400 });
  }
}
